...
// Map source filenames to version IDs
// s01_agent_loop.ts/.py -> s01
// s_full.ts/.py -> skipped (reference agent)
function filenameToVersionId(filename: string): string | null {
  const base = path.parse(filename).name;
  if (base === "s_full") return null;
  if (base === "_runtime") return null;
...
// Looks for name: "tool_name" or "name": "tool_name" patterns.
function extractTools(source: string): string[] {
  const toolPattern = /\b["']?name["']?\s*:\s*["']([\w-]+)["']/g;
...
function extractDocVersion(filename: string): string | null {
  ...
  // Exclude range/meta docs like "s01-s19-practice-checklist.md".
  // They are not single chapter docs and should be treated as bridge docs.
  const rest = filename.slice(m[0].length);
  if (/^s\d+[a-c]?-/i.test(rest)) {
    return null;
  }
...
function main() {
  ...
  const agentFiles = fs
    ...
    .filter((f) => f.startsWith("s") && (f.endsWith(".ts") || f.endsWith(".py")));
